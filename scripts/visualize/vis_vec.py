...
from utils.smpl_utils.smpl_skeleton import CHAIN_DICT, SMPLSkeleton, RAW_OFFSETS_DICT
from utils.vectorize.vec2xyz import recover_from_ric, recover_from_rot
...
def plot_3d_motion(save_path, kinematic_tree, joints: np.ndarray, title, figsize=(10, 10), fps=20, radius=4):
    matplotlib.use('Agg')
...
if __name__ == '__main__':
    ...
    args.add_argument('--fps', type=str, default=20)
    ...
    if joints.shape[-1] != 3:
        ...
        if args.use_rot:
            skeleton = SMPLSkeleton(offset=torch.from_numpy(
                np.asarray(RAW_OFFSETS_DICT[num_joints])),
                                    kinematic_tree=CHAIN_DICT[num_joints])
            ...
            joints = recover_from_rot(torch.from_numpy(joints[None, ...]).to(torch.float32), joints_num=num_joints).numpy()
    ...
    plot_3d_motion(args.save_path, CHAIN_DICT[num_joints], joints, args.title,fps=args.fps)
